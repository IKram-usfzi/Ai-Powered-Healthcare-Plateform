...
**7. This sandbox has no Docker, no Node/npm, no PostgreSQL, and no passwordless sudo — only Python 3.10 (+ pip, installed this session via `get-pip.py --user`)**
STATUS: Confirmed (checked directly: `apt-cache policy postgresql` shows it's installable but not installed; `sudo -ln` requires a password we don't have — did not attempt to bypass this). IMPACT: Medium — backend Python code (including SQLAlchemy/Alembic) can be written and actually run/tested here against SQLite as a logical stand-in (done: FastAPI health endpoint verified live; Alembic migration up/downgrade verified on SQLite; Synthea seed script verified end-to-end on SQLite). Frontend (`npm install`/`vite build`), the full `docker compose up` stack, and any real PostgreSQL-specific behavior (native ENUM types, concurrent access, etc.) cannot be executed or verified in this sandbox at all; they must be run on the student's own machine (16 GB MacBook, per `TRD.md` §2) or a different environment with Docker/Node/Postgres. NEXT ACTION: Always say plainly when something is written-but-unverified vs. actually run, per §8. Don't claim `docker compose up` or "migrated into a running PostgreSQL container" (Phase 1 exit criteria) works without the user (or a Docker-capable environment) actually confirming it.
