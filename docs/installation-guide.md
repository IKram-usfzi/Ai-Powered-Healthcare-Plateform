...
- **Every API request returns `403 Forbidden`, even for a valid administrator login:** OPA has no policies loaded — confirm with `curl http://localhost:8181/v1/policies` (should return the two files under `infra/opa/policies/`, not an empty `[]`). This happens if the `opa` container started before `infra/opa/policies/` existed on the host (e.g. right after a `git clone` into a directory Docker had already bind-mounted against) — Docker silently binds the container to an empty directory reference that doesn't update even after the real files appear. Fix: `docker compose -f infra/docker-compose.yml up -d --force-recreate --no-deps opa`, then re-check `/v1/policies`. `require_roles()` fails closed by design (`deccission.md` ADR-024) — this is the one situation where "everything is a 403" means an infra problem, not an auth bug.
